...
class BotTrainer(object):
    def __init__(self, corpus_dir):
        ...
        with self.graph.as_default():
            tokenized_data = TokenizedData(corpus_dir=corpus_dir)  #进行数据处理，使用tokenieddata类

            self.hparams = tokenized_data.hparams  #参数赋值
            self.train_batch = tokenized_data.get_training_batch()  # 获得batch
            self.model = ModelCreator(training=True, tokenized_data=tokenized_data,  #构造训练模型
    ...
    def train(self, result_dir, target=""):
        ...
        with tf.Session(target=target, config=config_proto, graph=self.graph) as sess:
            ...
            # Done training
            self.model.saver.save(sess, os.path.join(result_dir, "basic"), global_step=global_step)  #保存当前的模型
