...
class ModelCreator(object):
    ...
    def __init__(self, training, tokenized_data, batch_input, scope=None):
        ...
        self.num_layers = hparams.num_layers  #层数
        self.time_major = hparams.time_major  #

        
        initializer = model_helper.get_initializer(  #模型初始化为uniform格式
            hparams.init_op, hparams.random_seed, hparams.init_weight)
        tf.get_variable_scope().set_initializer(initializer)

        
        self.embedding = (model_helper.create_embbeding(vocab_size=self.vocab_size,  #将词变成高维向量，称为embedding
        ...
        with tf.variable_scope(scope or "build_network"):
            with tf.variable_scope("decoder/output_projection"):
                self.output_layer = layers_core.Dense(
                    self.vocab_size, use_bias=False, name="output_projection")  #找出词库中出现概率最大的词进行输出

        
        print("# Building graph for the model ...") #对模型进行图像构建
        ...
        if training:
            ...
            opt = tf.train.AdamOptimizer(self.learning_rate)  #优化器

            gradients = tf.gradients(self.train_loss, params) #计算梯度

            clipped_gradients, gradient_norm_summary = model_helper.gradient_clip(  #将梯度限制在一定范围内，称为截断
                gradients, max_gradient_norm=hparams.max_gradient_norm)

            self.update = opt.apply_gradients(  #参数更新
    ...
    def _build_encoder(self, hparams):
        ...
        if self.time_major:  #一般情况下time_major类型的固定语句
            source = tf.transpose(source)

        with tf.variable_scope("encoder") as scope:
            dtype = scope.dtype

            #检查embedding, emp_inp: [max_time, batch_size, num_units]
    ...
    def _build_decoder(self, encoder_outputs, encoder_state, hparams):
        ...
        bos_id = tf.cast(self.vocab_table.lookup(tf.constant(hparams.bos_token)), tf.int32)  #起始符号的ID
        eos_id = tf.cast(self.vocab_table.lookup(tf.constant(hparams.eos_token)), tf.int32)  #中止符号的ID
    ...
    def _build_decoder_cell(self, hparams, encoder_outputs, encoder_state,
        ...
        attention_mechanism = tf.contrib.seq2seq.LuongAttention(  #注意力机制
            num_units, memory, memory_sequence_length=source_sequence_length)

        cell = model_helper.create_rnn_cell(  #使用create_rnn_cell函数创键cell
            num_units=num_units,
        ...
        cell = tf.contrib.seq2seq.AttentionWrapper(  #tensorflow库中的seq网络里的attention机制
            cell,
            attention_mechanism,
    ...
    def _compute_loss(self, logits):
        ...
        loss = tf.reduce_sum(crossent * target_weights) / tf.to_float(self.batch_size)  #计算出损失值
        return loss
